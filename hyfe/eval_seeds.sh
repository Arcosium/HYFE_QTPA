...
cd /home/arcosium/projects/HYFE_QTPA; R=work/results
...
for f in $R/hold_i1_heatf_4h_W60_H84_s4_pred.npz $R/hold_i1_heatf_sd[0-9]_4h_W60_H84_s4_pred.npz; do [ -f "$f" ] && coh $f; done
fs=$(ls $R/hold_i1_heatf_4h_W60_H84_s4_pred.npz $R/hold_i1_heatf_sd[0-9]_4h_W60_H84_s4_pred.npz 2>/dev/null); n=$(echo $fs | wc -w)
python3 -m hyfe.zavg $fs --out $R/ens_heatf${n}_hold_4h_s4_pred.npz > /dev/null 2>&1 && coh $R/ens_heatf${n}_hold_4h_s4_pred.npz
python3 - <<'PY'
import json, glob, re, pandas as pd, numpy as np
from hyfe.perf import daily_returns, stats
R='work/results'; sd=lambda f:(re.search(r'_sd(\d+)_',f) or [None,'0'])[1]
def load(pat):
    return {(sd(f), re.search(r'_s(\d)_cohort',f)[1]): f for f in glob.glob(pat)}
full=load(f'{R}/full_i1_heatf*_liq_4h_W60_H84_s[0-3]_cohort_H84.json'); hold=load(f'{R}/hold_i1_heatf*_4h_W60_H84_s4_cohort_H84.json')
...
for sdn in seeds:
    ...
    f=hold.get((sdn,'4')); r['hold']=json.load(open(f))['summary']['sharpe'] if f else np.nan; rows.append(r)
...
for pat,lag,name in [(f'{R}/ens_heatf*_direct_4h_s[0-3]_cohort_H84.json',14,'폴드 앙상블'),(f'{R}/ens_heatf*_hold_4h_s4_cohort_H84.json',14,'홀드아웃 앙상블')]:
    g={}
