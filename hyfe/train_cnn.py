    ...
    def __init__(self, bases, res, W, H, k, device, start="2023-01", end="2026-02", label="ksigma", fixed=0.02, need_feats=False, stride=None, macro=False):
        ...
        for b in bases:
            ...
            if lw is None or not len(lw):
                continue
            if need_feats:
                ft = F.make(df, W, H, k, bars_per_30d=max(W, 30 * 1440 // B.RES_MIN[res]), mode=label, fixed=fixed)
            ...
            arrs.append(np.c_[df[["o", "h", "l", "c", "v"]].to_numpy(np.float32), ctx, mac])   # 열: o h l c v vrel30 hour dow btc_r24 btc_r7d breadth24
            lw["base"] = b; lw["g"] = lw.e + off       # 전체 배열에서의 창 끝 위치
            rows.append(lw)
        self.arr = torch.from_numpy(np.concatenate(arrs)).to(device)
...
class ResNet18(nn.Module):
    def __init__(self, n_cls=3):
        ...
        w = m.conv1.weight.sum(1, keepdim=True)
        m.conv1 = nn.Conv2d(1, 64, 7, 2, 3, bias=False); m.conv1.weight.data = w
...
def build_model(a, W):
    ...
    return {"i1": lambda: SmallCNN(in_ch=c), "i1f": lambda: SmallCNN(pool_w=2, in_ch=c), "i2": lambda: ResNet18(), "j2": lambda: SeqTransformer(W, in_dim=8 if a.seq_ctx else 5),
...
def predict(model, wins, g, f, a, bs=512):
    ...
    for i in range(0, len(g), bs):
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            out.append(torch.softmax(model(x, ff).float(), 1).cpu())
...
def main():
    ...
    ap.add_argument("--model", default="i1", choices=["i1", "i1f", "i2", "j2", "f1"])
    ...
    t0 = time.time()
    wins = Windows(bases, a.res, a.W, a.H, a.k, dev, a.start, a.test_end, a.label, a.fixed, need_feats=(a.model == "f1"), stride=a.stride or None, macro=a.channels.endswith("m"))
    ix = wins.idx
    ...
    feat = None
    if a.model == "f1":   # 표준화 통계는 학습 집합에서만
        mu, sd = tr[F.FEATURES].mean(), tr[F.FEATURES].std() + 1e-9
        feat = lambda d: torch.tensor(((d[F.FEATURES] - mu) / sd).to_numpy(np.float32), device=dev)
    f_tr, f_va, f_te = (feat(tr), feat(va), feat(te)) if feat else (None, None, None)

    model = build_model(a, a.W).to(dev)
    lr = a.lr or {"i1": 1e-3, "i1f": 1e-3, "i2": 1e-4, "j2": 5e-4, "f1": 1e-3}[a.model]
    ...
    for ep in range(a.epochs):
        ...
        for i in range(0, len(perm), a.bs):
            ...
            with torch.autocast("cuda", dtype=torch.bfloat16):
                loss = Fn.cross_entropy(model(x, ff).float(), y_trt[j])
            opt.zero_grad(set_to_none=True); loss.backward(); opt.step(); tl += loss.item() * len(j)
        pv = predict(model, wins, g_va, f_va, a); r = M.evaluate(va.label.to_numpy(), pv)
        hist.append({"epoch": ep, "loss": tl / len(perm), "val_ap": r["ap"], "sec": round(time.time() - te0)})
        print(f"ep{ep} loss {tl/len(perm):.4f} val_ap {r['ap']:.4f} lift {r['lift']:.2f} {time.time()-te0:.0f}s", flush=True)
        if r["ap"] > best:
