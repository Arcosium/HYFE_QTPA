...
# usage: [LABEL=relbin STRIDE=1 CHANNELS=gray NOVOL= DETREND= SEQCTX= LR= EPOCHS= CAP=5000000] bash hyfe/fulldir.sh j2 4h:60:42 [0 1 2 3]
set -u
cd "$(dirname "$0")/.."
...
for s in $folds; do
  ...
  python -m hyfe.train_cnn --res $res --W $W --H $H --model $model --channels $CHANNELS --label $LABEL --stride $STRIDE --bases @work/liq12_s$s.txt $EXTRA --cap $CAP --start 2023-01 --val $V --test $T --test_end $TE --out work/results/full_${model}${SUF}_${tag}_s${s}.json --save_pred 2>&1 | grep --line-buffered -E "train|^ep|DONE"
done
echo FULL_DONE
