...
**이미지 CNN(교사 없음, 소형 CNN 75만 파라미터, 4h·60봉·14일 상대부호 라벨, 사전 등록 시드 0~9 열 개 전부의 z-평균 앙상블)**
— 코호트 롱숏(매 4h 마감에 상·하위 10% 를 14일 보유, 달러 중립, 왕복 0.2%, 단순수익), 폴드별 유동성 상위 200:

| 검증 구간 | Sharpe | p (Newey-West) | MDD | 순자산 | GBM 같은 구간 | 점수 셔플 ×2 |
|------|------|------|------|------|------|------|
| 2024-06~08 | 5.59 | 0.009 | −2.3% | ×1.12 | 4.48 | −6.0 / −3.3 |
| 2024-12~2025-02 | 4.95 | 0.008 | −3.1% | ×1.17 | 2.68 | −2.3 / −0.3 |
| 2025-09~11 | 2.92 | 0.012 | −4.2% | ×1.16 | 1.54 | −1.4 / −0.1 |
| 2025-12~2026-02 | 2.28 | 0.096 | −3.3% | ×1.07 | 1.22 | −0.5 / −5.4 |
| 합산 408일 | **3.53** | **< 0.0001** (t 4.06) | −4.2% | ×1.63 | 2.18 | |
| 홀드아웃 2026-03~05 (설정 확정 뒤 첫 사용) | 2.49 | 0.019 | −7.9% | ×1.12 | 0.35~0.98 | −5.1 / −3.5 |

연구 데이터 창은 2026-05 까지다. 폴드는 모두 학습 종료 뒤 1~3개월을 재므로 홀드아웃도 같은 거리(2026-03~05)로 잰다.
시드 검증(선택 없음, 열 개 전부 보고): 시드 하나짜리 합산 Sharpe 는 2.29~3.64(평균 2.85, 표준편차 0.39, 열 개 모두 p ≤ 0.003),
가장 약한 폴드(2025-12~02)는 시드 열 개 중 아홉이 1 초과(최소 0.22). 홀드아웃 2026-03~05 는 시드별 1.39~4.56(열 개 전부 > 1.3, 여섯 개 p < 0.05).
**한계(부록)**: 알파는 학습 시점에서 멀어질수록 감쇠한다. 같은 모델을 2026-06~08(학습 뒤 4~6개월)까지 끌면 앙상블 0.61(p 0.32)로 떨어지고,
2026-02 까지 재학습해 03~05 로 검증한 뒤 06~08 을 시험한 롤링 폴드는 앙상블 1.51(p 0.15, 92일, 시드 열 개 전부 양수 0.47~2.20)로
양수지만 석 달로는 유의하지 않다(6·7월 −1.1%·−0.9%, 8월 +9.6%). 실전 운용은 분기 재학습을 전제로 한다. 이미지(`heatf`, 96×180 그레이스케일) = 창 안 OHLC 로그비·거래량·30일 대비
