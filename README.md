...
| 홀드아웃 2026-03~08 (설정 확정 뒤 첫 사용) | 1.54 | 0.045 | −8.2% | ×1.15 | 0.35~0.98 | −5.0 / −0.4 |

시드 검증(선택 없음, 열 개 전부 보고): 시드 하나짜리 합산 Sharpe 는 2.29~3.64(평균 2.85, 표준편차 0.39, 열 개 모두 p ≤ 0.003),
가장 약한 폴드(2025-12~02)는 시드 열 개 중 아홉이 1 초과(최소 0.22). 홀드아웃은 시드별 0.68~2.48(평균 1.51) 로 편차가 커
앙상블이 필수이고 처음 보고했던 시드 두 개 값 2.24 는 상단 편차였다. 이미지(`heatf`, 96×180 그레이스케일) = 창 안 OHLC 로그비·거래량·30일 대비
